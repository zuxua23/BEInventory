...
public class PrintTagRegisService : IPrintTagRegisService
{
    ...
    )
    {
        try
        {
            ...
            var location = await GetStagingLocationAsync();

            var tags = new List<Tag>();

            var histories = new List<HistoryPrint>();

            var printCommands = new List<string>();

            using var trx = await _db.Database.BeginTransactionAsync();

            try
            {
                var currentNumber = await GetCurrentRunningNumberAsync();
    ...
    private async Task PrintToPrinterAsync( string sbpl, string user )
    {
    ...
    private Tag CreateTag(  Item item, Location location, long runningNumber,  string user )
    {
        return new Tag
        {
    ...
    private HistoryPrint CreatePrintHistory( Tag tag, string batchNo, string user )
    {
    ...
    private HistoryPrint CreateRegisterHistory( Tag tag, string reference, string user )
    {
        return new HistoryPrint
        {
            Id = Guid.NewGuid().ToString(),
    ...
    public async Task<List<TagResponseDto>> GetAllTagRegisAsync()
    {
        try
        {
            DailyFileLogger.Info("Retrieving all active tags.");
    ...
    public async Task<TagStatusCountDto> GetTagStatusCountAsync()
    {
        try
        {
            DailyFileLogger.Info("Retrieving tag count by status.");

            var data = await _db.Tags
                .AsNoTracking()
                .Where(t => !t.IsDelete)
                .GroupBy(t => 1)
                .Select(g => new TagStatusCountDto
                {
                    InStock = g.Count(t =>
                        t.Status == TagStatus.IN_STOCK),

                    Reserved = g.Count(t =>
                        t.Status == TagStatus.RESERVED ||
                        t.Status == TagStatus.ALLOCATED),
                    ...
                    Others = g.Count(t =>
                        t.Status == TagStatus.PRINTED ||
                        t.Status == TagStatus.STANDBY),

                    Total = g.Count()
                })
                .FirstOrDefaultAsync();

            return data ?? new TagStatusCountDto();
        }
        catch (Exception ex)
        {
            DailyFileLogger.Error(
                "Failed to retrieve tag count by status.",
                ex
            );
    ...
    public async Task<List<StockResponseDto>> GetStockPerItemAsync()
    {
        try
        {
            ...
            var data = await _db.Items
                .AsNoTracking()
                .Where(i => !i.IsDelete)
                .Select(i => new StockResponseDto
                {
                    ItemId = i.Id,
                    ItemName = i.Name,
                    TotalStock = _db.Tags.Count(t =>
                        !t.IsDelete &&
                        t.ItemId == i.Id &&
                        t.Status == TagStatus.IN_STOCK
                    ),
