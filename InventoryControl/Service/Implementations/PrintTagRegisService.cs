...
PH";
    ...
    public async Task RegisterAsync(TagRegistrationDto dto, string user)
    {
        try
        {
            ...
            // Cari berdasarkan TagId ATAU EpcTag
            var tags = await _db.Tags
                .Where(t => dto.TagIds.Contains(t.TagId) || dto.TagIds.Contains(t.EpcTag))
                .ToListAsync();
            ...
            // Validasi missing tags — cek juga di dua field
            var foundIds = tags.Select(t => t.TagId).ToHashSet();
            var foundEpcs = tags.Select(t => t.EpcTag).ToHashSet();
            var missingTags = dto.TagIds
                .Where(id => !foundIds.Contains(id) && !foundEpcs.Contains(id))
                .ToList();
            ...
            foreach (var tag in tags)
            {
                if (tag.Status != "PRINTED" && tag.Status != "OUT")
                {
                    ...
                    throw new Exception($"Tag {tag.TagId} tidak bisa diregister (status: {tag.Status})");
                }
