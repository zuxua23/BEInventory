...
public async Task<byte[]> ExportExcel(DateTime? fromDate, DateTime? toDate, string? txType)
{
    var data = await GetHistory(fromDate, toDate, txType);
    ...
    {
        ws.Cell(row, 1).Value = d.TxDate?.ToString("yyyy-MM-dd") ?? "-";
        ws.Cell(row, 2).Value = d.TxType;
        ws.Cell(row, 3).Value = d.DoNumber;
        ws.Cell(row, 4).Value = d.ReaderName;
        ws.Cell(row, 5).Value = d.TagId;
        ws.Cell(row, 6).Value = d.ItemName;
        ws.Cell(row, 7).Value = d.LocationName;
    ...
    for (int i = 2; i < row; i++)
    {
        if (i % 2 == 0)
        {
            ws.Row(i).Style.Fill.BackgroundColor = XLColor.FromHtml("#f8fafc");
        }
    }
...
public async Task<byte[]> ExportCsv(DateTime? fromDate, DateTime? toDate, string? txType)
    {
        var data = await GetHistory(fromDate, toDate, txType);

        var sb = new StringBuilder();

        sb.AppendLine("Date,Type,DO Number,Reader,Tag ID,Item,Location");

        string Escape(string val) => $"\"{val?.Replace("\"", "\"\"")}\"";

        foreach (var d in data)
        {
            sb.AppendLine($"{d.TxDate:yyyy-MM-dd}," +
                          $"{Escape(d.TxType)}," +
                          $"{Escape(d.DoNumber)}," +
                          $"{Escape(d.ReaderName)}," +
                          $"{Escape(d.TagId)}," +
